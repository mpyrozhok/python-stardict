...
class IfoFileReader(object):
    ...
    def __init__(self, filename):
        ...
        with open(filename, "r") as ifo_file:
            ...
            if key != "version":
                raise IfoFileException("Version info expected in the second"
                                       " line of {!r:s}!".format(filename))
...
class IdxFileReader(object):
    """Read dictionary indexes from the .idx file and store the indexes in a
    list and a dictionary.
    The list contains each entry in the .idx file, with subscript indicating
    the entry's origin index in .idx file.
    The dictionary is indexed by word name, and the value is an integer or a
    ...
    def __init__(self, filename, compressed=False, index_offset_bits=32):
        ...
        for word_str, word_data_offset, word_data_size, index in self:
            self._index_idx.append((word_str, word_data_offset,
                                    word_data_size))
    ...
    def get_index_by_num(self, number):
        ...
        if number >= len(self._index_idx):
            raise IndexError("Index out of range!"
                             " Acessing the {:d} index"
                             " but totally {:d}".format(number,
                                                        len(self._index_idx)))
    ...
    def get_index_by_word(self, word_str):
        ...
        Return:
            Index infomation corresponding to the specified word if exists,
            otherwise False.
        ...
        for k in range(0, len(sametypesequence)):
            if sametypesequence[k] in "mlgtxykwhnr":
                if k == len(sametypesequence) - 1:
                    result[sametypesequence[k]] = self._get_entry_field_size(size -
